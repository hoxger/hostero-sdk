...
func renderFromDictField(f Field) string {
	...
	case CodecMapEnum:
		if f.Required && !f.Nullable {
			return fmt.Sprintf("{k: (%s(v) if v is not None else None) for k, v in data[%s].items()}", f.TargetType, jsonKey)
		}
		return fmt.Sprintf("{k: (%s(v) if v is not None else None) for k, v in data[%s].items()} if data.get(%s) is not None else None", f.TargetType, jsonKey, jsonKey)
	default:
		if f.Required && !f.Nullable {
...
func renderToDictField(f Field) string {
	...
	case CodecMapEnum:
		return fmt.Sprintf("        if self.%s is not None:\n            result[%s] = {k: (v.value if v is not None else None) for k, v in self.%s.items()}", name, jsonKey, name)
	default:
